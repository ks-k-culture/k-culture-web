...
import { getGetJobsQueryKey, useCreateJob } from "@/src/jobs/jobs";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    createJob(
      ...
      },
      {
        ...
        onError: (error) => {
          console.error("구인글 작성 실패:", error);
          const err = error as unknown as Error & { status?: number };
          if (err.status === 401) {
            setSubmitError("로그인이 필요합니다. 다시 로그인해주세요.");
          } else {
            setSubmitError(err.message || "구인글 등록에 실패했습니다. 다시 시도해주세요.");
          }
        },
      }
    );
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        <div>
          <h1 className="text-ivory text-2xl font-bold">구인글 작성</h1>
          <p className="text-muted-gray mt-1">작품에 필요한 배우를 찾아보세요</p>
        </div>
