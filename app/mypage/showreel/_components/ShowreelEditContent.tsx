...
function FormSkeleton() {
  return (
    <main className="flex-1 pb-32 animate-pulse">
      ...
      {[1, 2, 3].map((i) => (
        <section key={i} className="px-5 py-4 border-b" style={{ borderColor: "#E5E8EB" }}>
      ...
      <div className="relative w-full max-w-lg bg-white min-h-screen flex flex-col">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white px-5 py-6 max-w-lg mx-auto">
          <button
            ...
            className="w-full py-4 rounded-xl font-medium text-white transition-colors disabled:cursor-not-allowed"
            style={{
              backgroundColor: isAddEnabled && !isCreating ? "#191F28" : "#E5E8EB",
              color: isAddEnabled && !isCreating ? "#FFFFFF" : "#B0B8C1",
            }}
          >
            {isCreating ? "추가중..." : "추가하기"}
          </button>
